...
#define symlink_name L"\\??\\mmmapiospace"
#define device_name L"\\device\\mmmapiospace"

#define MMMAPIOSPACE CTL_CODE(FILE_DEVICE_UNKNOWN, 0x800, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define SET_GLOBAL CTL_CODE(FILE_DEVICE_UNKNOWN, 0x804, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
NTSTATUS MyDispatcher(PDEVICE_OBJECT device_object, PIRP irp)
{
    ...
    switch (irp_stack->MajorFunction)
    {
        case IRP_MJ_DEVICE_CONTROL:
            ...
            {
                case MMMAPIOSPACE:
                    MmMapIoSpace(*(PHYSICAL_ADDRESS *)device_object->DeviceExtension,
                                 *(SIZE_T *)((char *)device_object->DeviceExtension + 8), MmNonCached);
                    break;
                case SET_GLOBAL:
                    *(PHYSICAL_ADDRESS *)device_object->DeviceExtension = *(PHYSICAL_ADDRESS *)inputBuffer;
                    *(SIZE_T *)((char *)device_object->DeviceExtension + 8) = *(SIZE_T *)(inputBuffer + 8);
                    break;
                default:
                    break;
            }
