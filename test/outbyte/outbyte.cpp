...
#define symlink_name L"\\??\\outbyte"
#define device_name L"\\device\\outbyte"

#define OUTBYTE CTL_CODE(FILE_DEVICE_UNKNOWN, 0x800, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define OUTWORD CTL_CODE(FILE_DEVICE_UNKNOWN, 0x804, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define OUTDWORD CTL_CODE(FILE_DEVICE_UNKNOWN, 0x808, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
NTSTATUS MyDispatcher(PDEVICE_OBJECT device_object, PIRP irp)
{
    ...
    switch (irp_stack->MajorFunction)
    {
        case IRP_MJ_DEVICE_CONTROL:
            ...
            {
                case OUTBYTE:
                    __outbyte(*(USHORT *)inputBuffer, *(UCHAR *)(inputBuffer + 8));
                    break;
                case OUTWORD:
                    __outword(*(USHORT *)inputBuffer, *(USHORT *)(inputBuffer + 8));
                    break;
                case OUTDWORD:
                    __outdword(*(USHORT *)inputBuffer, *(ULONG *)(inputBuffer + 8));
                    break;
                default:
                    break;
            }
