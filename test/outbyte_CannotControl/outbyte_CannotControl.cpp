...
#define OUTBYTE_CANNOT_CONTROL CTL_CODE(FILE_DEVICE_UNKNOWN, 0x800, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
NTSTATUS MyDispatcher(PDEVICE_OBJECT device_object, PIRP irp)
{
    ...
    switch (irp_stack->MajorFunction)
    {
        case IRP_MJ_DEVICE_CONTROL:
            ...
            {
                case OUTBYTE_CANNOT_CONTROL:
                    if (*(USHORT *)inputBuffer == 0xcf8)
                    {
