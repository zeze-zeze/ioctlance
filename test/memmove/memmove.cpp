...
#define symlink_name L"\\??\\memmove"
#define device_name L"\\device\\memmove"

#define MEMMOVE CTL_CODE(FILE_DEVICE_UNKNOWN, 0x800, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define MEMMOVE_DEREFERENCE CTL_CODE(FILE_DEVICE_UNKNOWN, 0x801, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define MEMMOVE_MMISADDRESSVALID CTL_CODE(FILE_DEVICE_UNKNOWN, 0x802, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define MEMMOVE_PROBEFORWRITE CTL_CODE(FILE_DEVICE_UNKNOWN, 0x803, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define MEMMOVE_PROBEFORREAD CTL_CODE(FILE_DEVICE_UNKNOWN, 0x804, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define MEMMOVE_BUFFER_OVERFLOW CTL_CODE(FILE_DEVICE_UNKNOWN, 0x805, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
NTSTATUS MyDispatcher(PDEVICE_OBJECT device_object, PIRP irp)
{
    ...
    switch (irp_stack->MajorFunction)
    {
        case IRP_MJ_DEVICE_CONTROL:
            ...
            {
                case MEMMOVE:
                    memmove(tmp, inputBuffer, 4);
                    break;
                case MEMMOVE_DEREFERENCE:
                    memmove(tmp, *(PVOID *)inputBuffer, 4);
                    break;
                case MEMMOVE_MMISADDRESSVALID:
                    MmIsAddressValid(inputBuffer);
                    memmove(tmp, inputBuffer, 4);
                    break;
                case MEMMOVE_PROBEFORWRITE:
                    ProbeForWrite(inputBuffer, 8, 8);
                    memmove(inputBuffer, tmp, 4);
                    break;
                case MEMMOVE_PROBEFORREAD:
                    ProbeForRead(inputBuffer, 8, 8);
                    memmove(tmp, inputBuffer, 4);
                    break;
                case MEMMOVE_BUFFER_OVERFLOW:
                    memmove(tmp, inputBuffer, *(size_t *)(inputBuffer + 8));
                    break;
                default:
                    break;
            }
